...
ROBUST_TARGETS = {
    "candidate_audit_robust_hybrid_v2": "hybrid_targeted",
    "candidate_audit_robust_old_targeted_v2": "old_full_targeted",
    "candidate_audit_robust_profile_v2": "hybrid_plus_short_answer",
}
...
def split_original_rows(records: list[dict[str, Any]], *, val_ratio: float = 0.2) -> list[str]:
    ...
    for record in records:
        value = int(sha256_hexdigest("audit-robust-v2-split", record.get("prompt"), record.get("answer"), record.get("label"))[:8], 16) / 0xFFFFFFFF
...
def _fixture(kind: str, base: AuditPayload | None = None) -> AuditPayload:
    ...
    neutral = {
        ...
        "conf": 0.5, "ok": True, "status": "ok", "model_name": "audit_robust_v2_aug", "mode": "dataset",
    }
    if kind == "neutral_dropout":
        neutral.update({"ok": False, "status": "cache_miss"})
    elif kind == "correct_high_noisy_x0":
        neutral.update({"h": 0.34, "u": 0.80, "we": 0.88, "wn": 0.80, "ue": 0.75, "bt": 0.82, "x": 0.0, "conf": 0.88})
    elif kind == "wrong_neutral":
        neutral.update({"h": 0.04, "u": 0.03, "we": 0.0, "wn": 0.0, "ue": 0.0, "bt": 0.0, "conf": 0.62})
    elif kind == "wrong_strong_contradiction":
        neutral.update({"h": 0.92, "u": 0.86, "we": 0.90, "wn": 0.86, "ue": 0.72, "bt": 0.68, "x": 1.0, "conf": 0.92})
    elif kind == "clean_audit_long":
        neutral.update({"h": 0.0, "u": 0.0, "we": 0.0, "wn": 0.0, "ue": 0.0, "bt": 0.0, "conf": 0.76, "c": 3.0})
    return AuditPayload.model_validate(neutral)


def augmentation_plan(label: int) -> list[tuple[str, float]]:
    plan = [("original", 1.50), ("neutral_dropout", 0.20), ("clean_audit_long", 0.60)]
    if int(label) == 0:
        plan.append(("correct_high_noisy_x0", 0.45))
    else:
        plan.extend([("wrong_neutral", 0.55), ("wrong_strong_contradiction", 0.25)])
    return plan
...
def _matrices(frame: pd.DataFrame, extractor: FeatureExtractor) -> tuple[np.ndarray, np.ndarray, np.ndarray]:
...
def train_one(frame: pd.DataFrame, output_dir: str | Path, feature_set: str) -> dict[str, Any]:
    ...
    X_train, y_train, w_train = _matrices(train_frame, extractor)
    X_val, y_val, w_val = _matrices(val_frame, extractor)
...
def train_audit_robust_candidates_v2(
    ...
    *,
    ...
    (Path(output_root) / "audit_robust_training_v2.summary.json").write_text(json.dumps(overall, indent=2, ensure_ascii=False), encoding="utf-8")
    return overall


def main() -> None:
    parser = argparse.ArgumentParser(description="Train v2 audit-robust candidates with distribution-anchored augmentations.")
